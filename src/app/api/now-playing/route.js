...
export async function GET() {
    try {
        ...
        const url = `${TMDB_BASE_URL}/movie/now_playing?api_key=${apiKey}&language=en-US&page=1`;
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: errorData.status_message || "Failed to fetch now playing movies" }, { status: response.status });
        }
        ...
        return NextResponse.json({ movies: nowPlayingMovies });
